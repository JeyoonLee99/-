...
def Correlation_Filter(df, target_variable, correlation_threshold):
    ...
    selected_columns = list(filtered_correlation.index[0:1])  # 상관계수가 높은 상위 N개 변수 선택
...
class LSTM(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, num_layers=1, l2_lambda=0.001):
...
def create_lstm_model(input_shape):
    input_size = input_shape[2]  # For LSTM, input_size is the number of features
...
# 학습 함수
def train_model(model, input_train, output_train, num_epochs=1000, batch_size=32, validation_split=0.15):
...
for i in tqdm(range(len(vif_results)), desc='LSTM modeling process'):
    ...
    output_np = np.array(output_df).reshape(-1, 1)  # Ensure output is the correct shape
    
    # Reshape input for LSTM [samples, time steps, features]
    input_np = input_np.reshape(input_np.shape[0], 1, input_np.shape[1])
    
    input_train, input_test, output_train, output_test = train_test_split(input_np, output_np, test_size=0.2, shuffle=False, random_state=0)
    
    model = create_lstm_model(input_train.shape)
    input_train_tensor, output_train_tensor = prepare_data(input_train, output_train)
    
    train_losses, val_losses = train_model(model, input_train_tensor, output_train_tensor)
    ...
    # Replace Do_scatter with appropriate function call
...
# Save the loss records to a file
all_losses_df = pd.concat(loss_records, ignore_index=True)
all_losses_df.to_csv('training_losses.csv', index=False)
...
input_test_ = input_test_.reshape(input_test_.shape[0], 1, input_test_.shape[1])
# output_test_ = output_test_.reshape(output_test_.shape[0], 1, output_test_.shape[1])

model = create_lstm_model(input_test_.shape)
model.load_state_dict(torch.load('model_' + selected_benchmark_model + '.pth'), strict=False)
...
def compute_metrics(output_true, output_pred):
    r2 = r2_score(output_true, output_pred)
    mse = mean_squared_error(output_true, output_pred)
    euclidean_distance = np.sqrt(np.sum((output_true - output_pred) ** 2, axis=1))
    std_to_y_equals_x = np.std(euclidean_distance)
    return r2, mse, std_to_y_equals_x

# 실제값과 예측값 사이의 거리의 표준 편차(std)를 계산
r2, mse, std_to_y_equals_x = compute_metrics(pd.DataFrame(output_test_), pd.DataFrame(predictions_test))

print(f'R2: {r2:.4f}')
print(f'MSE: {mse:.4f}')
print(f'Std to y=x: {std_to_y_equals_x:.4f}')

#%%

def plot_loss_records(loss_records):
    plt.figure(figsize=(15, 10))
    for loss_df in loss_records:
        output_variable = loss_df['Output_Variable'].iloc[0]
        plt.plot(loss_df['Epoch'], loss_df['Train_Loss'], label=f'{output_variable} - Train Loss')
        plt.plot(loss_df['Epoch'], loss_df['Val_Loss'], label=f'{output_variable} - Val Loss', linestyle='--')
...
# 로스 기록 시각화 함수 호출
plot_loss_records(loss_records)
